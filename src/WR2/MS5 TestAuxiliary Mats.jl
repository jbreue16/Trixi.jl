...
function WR2_initial_condition_polynomial(x, t, equations::Trixi.AbstractEquations)
    return SVector(2, x[2], x[2]^2, x[2]^3)
end
...
function ABL_initial_condition_polynomial(x, t, equations::Trixi.AbstractEquations)
    return SVector(0, 1, 2*x[2], 3*x[2]^2)
end
solver = DGSEM(basis, surface_flux, volume_integral)
# initial_condition = ABL_initial_condition_convergence_test
initial_condition = ABL_initial_condition_polynomial # ABL_initial_condition_convergence_test # ABL_initial_condition_polynomial # 
...
for vgl = 1:4
    println("conservative variable $vgl max error: ", maximum(abs.(u_ABL[vgl, :, :, :]-q2[vgl, :, :, :])))
end